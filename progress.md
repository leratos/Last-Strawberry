...
- G300-G349 Draft (gestartet): Vertical-Slice-Ausbau mit allgemeinen Quest-Flags (persistente Story-/Zustandsflags) und World-Pack-Authoring-Basis fuer die erste bespielbare Welt (Urban Occult), inkl. Context/API/UI-Sichtbarkeit und Regressionstests.

- G300-G349 abgeschlossen: Allgemeine Story-/Quest-Flags (persistente Welt-/Charakter-Flags) + World-Pack-Authoring-Basis fuer Urban Occult integriert, inkl. Context/API/UI-Sichtbarkeit und Regressionstests.
- Neue World-Pack-Authoring-Basis (pps/game_api/app/services/world_pack_authoring.py): AuthoredWorldPack-Registry (Urban Occult + Generic), Starter-Story-Flags und Context-Metadaten (pack_id, ersion, display_name, genre).
- WorldSeed erweitert um world_pack_id, world_pack_version, world_pack_display_name; Bootstrap-Preview schreibt diese Felder fuer Urban-Occult bzw. Generic-Starterwelten.
- Persistente Story-Flags eingefuehrt: Migration  08_g300_world_story_flags.sql, Repository-Methoden get_world_story_flags() + Upsert/Read-Helper in pps/game_api/app/persistence.py.
- Quest-/Flag-Verknuepfung: derive_story_flags_from_quests() in pps/game_api/app/services/quest_authoring.py setzt MVP-Flags (kael_interviewed, supply_crate_inspected, mira_report_completed, itual_leads_quest_completed, ... ) aus Queststatus/Objectives.
- Context-API erweitert: GameContextResponse liefert jetzt world_pack und story_flags; _assemble_context_for_world() liest Story-Flags aus Persistenz und haengt questbezogene Dialoghinweise weiter an 	arget_catalog.npcs[].discovery_state.
- Web-UI erweitert (pps/web_client/src/App.tsx, pps/web_client/src/api.ts): Pack-Anzeige im Story-Header sowie Story-Flags (MVP) im Charaktersheet, damit Quest-/Storyzustand fuer interne Playtests sichtbar ist.
- G260-Questtest erweitert: validiert jetzt zusaetzlich World-Pack-Genre + Story-Flag-Fortschritt ueber Kael/Kiste/Mira (kael_interviewed, supply_crate_inspected, mira_report_completed, itual_leads_quest_completed).
- Tests/Builds: pytest apps/game_api/tests/test_preview_routes.py::TestGameApiPreviewRoutes::test_g260_urban_occult_starter_quest_progresses_via_kael_crate_mira -q (1 passed), pytest apps/game_api/tests/test_preview_routes.py -q (35 passed), 
pm.cmd --prefix apps/web_client run build, finale Regression pytest backend_v2/tests backend_server/tests packages/shared_schemas/tests packages/rules_engine/tests apps/game_api/tests -q -> 320 passed.
- Offene Punkte / technische Schuld: Story-Flags werden aktuell hauptsaechlich aus Queststatus abgeleitet (MVP) und noch nicht als allgemeiner Event-/World-State-Composer gepflegt. Fuer authored Weltpakete spaeter erweitern auf regelgetriebene Flag-Transitions (Dialog, Kampf, Entdeckung, Fraktionen) mit dedizierter Transition-Engine.
